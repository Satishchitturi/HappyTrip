...
<script type="text/javascript">
	...
	function updateSelected(type, flightid) {
		...
		$('#' + type + '_airline_name').html(
				$('#airlinename_' + flightid).html());
		$('#' + type + '_flight_date').html(
				$('#scheduleddate_' + flightid).html());
		$('#' + type + '_flight_departure_time').html(
				$('#departuretime_' + flightid).html());
		$('#' + type + '_flight_arrival_time').html(
				$('#arrivaltime_' + flightid).html());
		...
		$('#' + type + '_airline_logo').addClass(
				$('#airlinelogo_' + flightid).html());
	}
</script>
</head>
		...
		<div class="Container">
			<div id="ContentFrame" class="clearfix">
					...
					<div id="SearchParams">
						<div id="SPRow">
							...
							<ul class="inline">
								...
								<c:if test="${SearchData.returnDate!=null}">
									<li class="no_border">, ${toCity} - ${fromCity}</li>
								</c:if>
								<li><fmt:formatDate value="${SearchData.departureDate}" />
									<c:if test="${SearchData.returnDate!=null}">, <fmt:formatDate
											value="${SearchData.returnDate}" />
									</c:if> <c:if test="${SearchData.noOfAdults>0}">, ${SearchData.noOfAdults} Adult</c:if>
				...
				<div class="Right clearfix">
					<div class="col clearfix">
						...
						<div id="SelectionInfo" class="clearfix" style="">
							...
							<div id="CurrentSelection" class="clearfix deal" style="">
								...
								<form:form action="bookSelectedFlights.html" method="post"
									commandName="SelectedFlights">


									<div id="button">
										<c:if
											test="${(toFlights != null && fn:length(toFlights) gt 0) 
											 || (returnFlights !=null && fn:length(returnFlights) gt 0 )}">
									...
									<div id="selectedflights" style="">
										<c:if test="${SelectedFlights.selectedReturnFlight != null }">
											...
											<div id="selectedreturn" class="flight clearfix">
												<form:hidden id="return_flightid"
													path="selectedReturnFlight.flightRouteId" />
												<div
													class="airline_logos ${SelectedFlights.selectedReturnFlight.flight.airline.airlineCode}"
													id="return_airline_logo"></div>
												<h3>
													<span id="return_airline_name">${SelectedFlights.selectedReturnFlight.flight.airline.airlineName}</span>
												</h3>
												<span id="return_flight_date"><fmt:formatDate
														value="${SelectedFlights.selectedReturnFlight.scheduledFlightDate}" /></span><br>
												<span class="departs" id="return_flight_departure_time">${SelectedFlights.selectedReturnFlight.departureTime}</span>
												- <span class="arrives" id="return_flight_arrival_time">${SelectedFlights.selectedReturnFlight.arrivalTime}</span><br>
												<span id="return_flight_cost"> <c:forEach var="cost"
														items="${SelectedFlights.selectedReturnFlight.flightRouteCosts}"
														varStatus="loopCount">
										...
										<c:if
											test="${SelectedFlights.selectedOutboundFlight != null }">
											<div id="selectedoutbound" class="flight clearfix">
												<form:hidden id="outbound_flightid"
													path="selectedOutboundFlight.flightRouteId" />
												<div
													class="airline_logos  ${SelectedFlights.selectedOutboundFlight.flight.airline.airlineCode}"
													id="outbound_airline_logo"></div>
												<h3>
													<span id="outbound_airline_name">${SelectedFlights.selectedOutboundFlight.flight.airline.airlineName}</span>
												</h3>
												<span id="outbound_flight_date"><fmt:formatDate
														value="${SelectedFlights.selectedOutboundFlight.scheduledFlightDate}" /></span><br>
												...
												<span id="outbound_flight_cost"> <c:forEach
														var="cost"
														items="${SelectedFlights.selectedOutboundFlight.flightRouteCosts}"
														varStatus="loopCount">
														<c:if test="${loopCount.count eq 1}">
															<td rowspan="1">${cost.costPerTicket}</td>
														</c:if>
					...
					<div id="universalDiv" class="universal">
						...
						<div class="sector" id="outbound_div">
							...
							<c:if test="${toFlights ==null || fn:length(toFlights) lt 1}">
								<div id="NoResults">
									<h2>Oops, we couldn't find any itinerary that matches your
										filter criteria.</h2>
								...
								<table>
									...
									<tbody class="selected">
										...
										<c:forEach var="flight" items="${toFlights}" varStatus="count">
											<c:forEach var="cost" items="${flight.flightRouteCosts}"
												varStatus="loopCount">
												<c:if test="${loopCount.count eq 1}">
											...
											<span style="display: none"
												id="airlinename_${flight.flightRouteId}">${flight.flight.airline.airlineName}</span>
											<span style="display: none"
												id="scheduleddate_${flight.flightRouteId}"><fmt:formatDate
													value="${flight.scheduledFlightDate}" /></span>
											<span style="display: none"
												id="departuretime_${flight.flightRouteId}">${flight.departureTime}</span>
											<span style="display: none"
												id="arrivaltime_${flight.flightRouteId}">${flight.arrivalTime}</span>
											<span style="display: none"
												id="totalcost_${flight.flightRouteId}">${totalCost}</span>
											<span style="display: none"
												id="airlinelogo_${flight.flightRouteId}">${flight.flight.airline.airlineLogo}</span>

											<tr id="${flight.flightRouteId}">
												<td rowspan="2" class="button"><c:if
														test="${count.count eq 1 }">
														<input type="radio" name="departureJourneyId"
															checked="checked"
															onclick="selectOutbound('${flight.flightRouteId}')">
													</c:if> <c:if test="${count.count > 1 }">
														<input type="radio" name="departureJourneyId"
															onclick="selectOutbound('${flight.flightRouteId}')">
													</c:if></td>
					...
					<c:if test="${SearchData.returnJourney}">
					...
					<c:if
						test="${SearchData.returnJourney && (returnFlights ==null || fn:length(returnFlights) lt 1) }">
						<div id="NoResults">
							<h2>Oops, we couldn't find any return flight that matches
								your filter criteria.</h2>
						</div>
					</c:if>
					<c:if
						test="${returnFlights !=null && fn:length(returnFlights) gt 0 }">
							...
							<table>
								...
								<tbody class="selected">
									...
									<c:forEach var="flight" items="${returnFlights}"
										...
										<tr id="${flight.flightRouteId}">
											<td rowspan="2" class="return button"><c:if
													test="${count.count eq 1}">
													<input type="radio" checked="checked"
														name="arrivalJourneyId"
														onclick="selectReturn('${flight.flightRouteId}')">
												</c:if> <c:if test="${count.count > 1}">
													<input type="radio" name="arrivalJourneyId"
														onclick="selectReturn('${flight.flightRouteId}')">
												</c:if></td>
											<td rowspan="2" class="airline_logo"><div
													class="airline_logos ${flight.flight.airline.airlineLogo}"></div></td>
