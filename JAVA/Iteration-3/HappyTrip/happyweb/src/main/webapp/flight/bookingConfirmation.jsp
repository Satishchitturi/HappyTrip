							...
							<form id="universal_display" action="initiate-booking"
								...
								<div id="universalDiv" class="universal">
									...
									<div class="sector" style="width:100%" id="return_div">
										...
										<div class="sector_info" style="font-size: 13px;">
											<div class="marginText"><b>Reference Number : </b>${Bookings.referenceNumber}</div>
										</div>
										<div id="return">
											<table>
												<colgroup>
												...
												<tbody class="selected detailRow">
													<tr>
														...
														<td>${SelectedFlights.selectedOutboundFlight.flight.flightName}</td>
														<td>${SearchData.flightClass}</td>
														<td rowspan="1">${SearchData.fromCity}-${SearchData.toCity}</td>
														<td rowspan="1">${Bookings.outboundFlightBooking.noOfSeats}</td>
														<td rowspan="1">${Bookings.outboundFlightBooking.insuranceAmount}</td>
														<td rowspan="1">${Bookings.outboundFlightBooking.miles}</td>
												...
												<tbody class="selected detailRow">
													<tr>
														...
														<td>${SelectedFlights.selectedReturnFlight.flight.flightName}</td>
														<td>${SearchData.flightClass}</td>
														<td rowspan="1">${SearchData.toCity}-${SearchData.fromCity}</td>
														<td rowspan="1">${Bookings.returnFlightBooking.noOfSeats}</td>
														<td rowspan="1">${Bookings.returnFlightBooking.insuranceAmount}</td>
														<td rowspan="1">${Bookings.returnFlightBooking.miles}</td>
											...
											<table>
												...
												<tbody class="selected detailRow">
													...
													<tr>
														<td style="text-align: center">${passenger.title}</td>
														<td style="text-align: center">${passenger.name}</td>
											...
											<c:if test="${SelectedFlights.selectedReturnFlight != null}">
                                                <div class="detailRow marginText"><b>Total Price Including Insurance : </b>Rs <fmt:formatNumber maxFractionDigits="2" value="${Bookings.outboundFlightBooking.booking.totalCost +  Bookings.returnFlightBooking.booking.totalCost}"/> </div>
                                            </c:if>
                                            <c:if test="${SelectedFlights.selectedReturnFlight == null}">
                                            	<div class="detailRow marginText"><b>Total Price Including Insurance : </b>Rs <fmt:formatNumber maxFractionDigits="2" value="${Bookings.outboundFlightBooking.booking.totalCost}"/> </div>
                                            </c:if>
										</div>
									</div>
