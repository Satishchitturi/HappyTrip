...
public class FlightServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		flight.setFlightName(request.getParameter("flight"));
		...
		if(null != session && null != session.getAttribute("user") && ((User)session.getAttribute("user")).getEmail().equalsIgnoreCase("happyadmin@mindtree.com")){
			request.getRequestDispatcher("addFlightConfirm").forward(request, response);
		} else {
