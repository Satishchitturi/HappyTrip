...
import com.happytrip.dao.jpa.JpaCityDao;
...
public class HomeController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp)
		...
		if (req.getRequestURI().endsWith("/home.html")) {
			...
			out.print(obj);
			out.flush();
		} else if (req.getRequestURI().endsWith("/getAllCities")) {
			System.out.println("getting all the cities list");
			List<City> cities = new JpaCityDao().getAllCities();
			PrintWriter out = resp.getWriter();
			resp.setContentType("application/json");
			JSONArray obj = (JSONArray) JSONSerializer.toJSON(cities);
			resp.setContentType("application/json");
