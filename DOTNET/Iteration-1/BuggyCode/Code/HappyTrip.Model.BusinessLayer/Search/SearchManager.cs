    ...
    class SearchManager : HappyTrip.Model.BusinessLayer.Search.ISearchManager
    {
        ...
        private bool ValidateSearchInformation(SearchInfo searchInformation)
        {
            ...
            if (searchInformation.Direction == TravelDirection.OneWay)
            {
                ...
                if (searchInformation.OnwardDateOfJourney == null)
                    isValid = false;
            ...
            {
                if (searchInformation.OnwardDateOfJourney == null || searchInformation.ReturnDateOfJourney == null)
                    // Onward Date Of Journey and Retrun Date Of Journey cannot be null
                    isValid = false;

                if (searchInformation.ReturnDateOfJourney == dt)
                    isValid = false;

                if (searchInformation.ReturnDateOfJourney.Date <= DateTime.Now.Date)
                    // Onward Date is less than the Current Date
                    isValid = false;

                if (searchInformation.OnwardDateOfJourney >= searchInformation.ReturnDateOfJourney)
