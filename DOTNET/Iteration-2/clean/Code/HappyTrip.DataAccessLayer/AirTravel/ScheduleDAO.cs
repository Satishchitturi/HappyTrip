...
namespace HappyTrip.DataAccessLayer.AirTravel
{
	...
	class ScheduleDAO : DAO, IScheduleDAO
	{
						...
						using (IDataReader reader2 = GetDatabaseConnection().ExecuteReader("getScheduleFlightCost", schedule.ID))
						{
							try
							{
								while (reader2.Read())
								{
									...
									classCost.CostPerTicket = Convert.ToDecimal(reader2["CostPerTicket"].ToString());
		...
		public int UpdateScheduleFlightCost(Schedule scheduleInfo, FlightCost flightCostInfo)
		{
			...
			{
				db = GetDatabaseConnection();
				return db.ExecuteNonQuery("UpdateFlightCost", scheduleInfo.ID, flightCostInfo.Class, flightCostInfo.CostPerTicket);
			}
			catch (Common.ConnectToDatabaseException)
			{
